...
def metrics_summary(db: Session = Depends(get_db)) -> dict:
    ...
    # 7. Peak Hours Heatmap (Mon–Fri, 09:00–15:00 working hours)
    # Use created_at as approximation for arrival time.
    heat_rows = (
        db.query(
            func.strftime("%w", models.Token.created_at).label("weekday"),
            func.strftime("%H", models.Token.created_at).label("hour"),
            func.count(models.Token.id).label("count"),
        )
        .filter(models.Token.created_at >= week_start)
    ...
    # Hourly slots for 08:00 – 16:00 (8 AM to 4 PM)
    slot_definitions = [
        ("08:00", range(8, 9)),
        ("09:00", range(9, 10)),
        ("10:00", range(10, 11)),
    ...
    heatmap_values: dict[tuple[str, str], int] = {}
    for weekday_str, hour_str, count in heat_rows:
        try:
            weekday_idx = int(weekday_str)
            hour = int(hour_str)
        except (TypeError, ValueError):
            continue
        day_label = weekday_map.get(weekday_idx)
        if day_label is None:
            continue
        slot_label = None
        for label, hours in slot_definitions:
            if hour in hours:
                slot_label = label
                break
        if slot_label is None:
            continue
        key = (day_label, slot_label)
        heatmap_values[key] = heatmap_values.get(key, 0) + int(count or 0)

    heatmap_data = [
        {"day": day, "slot": slot, "count": heatmap_values.get((day, slot), 0)}
        for day in day_labels
        for slot, _hours in slot_definitions
    ]

    # 8. Counter Performance Comparison (last 7 days)
    counters = db.query(models.Counter).order_by(models.Counter.id.asc()).all()
    counter_performance = []
    for counter in counters:
        tokens_q = (
            ...
            .filter(
                ...
                models.Token.created_at >= week_start,
            )
            .all()
        )
        if not tokens_q:
            counter_performance.append(
                {
                    "name": counter.name,
                    "served": 0,
        ...
        served_tokens = [t for t in tokens_q if t.status == "COMPLETED"]
        cancelled_tokens = [t for t in tokens_q if t.status == "CANCELLED"]
        total_finished_c = len(served_tokens) + len(cancelled_tokens)

        # Average wait for served tokens
        total_wait_seconds_c = 0.0
        for t in served_tokens:
            ...
            total_wait_seconds_c += (end - start).total_seconds()
        avg_wait_minutes_c = (
            (total_wait_seconds_c / len(served_tokens)) / 60.0 if served_tokens else 0.0
        )

        completion_rate_c = (
            (len(served_tokens) / total_finished_c * 100.0) if total_finished_c > 0 else 0.0
        )

        counter_performance.append(
            {
                "name": counter.name,
                "served": len(served_tokens),
                "avg_wait_minutes": round(avg_wait_minutes_c, 1),
                "completion_rate": round(completion_rate_c, 1),
            }
        )
    ...
    return {
        "kpi": [
            ...
            {
                "label": "Fairness Ratio (β)",
                "value": f"{fairness_ratio:.2f}",
                "trend": "0.00",
                "positive": True,
        ...
        "heatmap": {
            "days": day_labels,
            "slots": [label for label, _ in slot_definitions],
            "values": heatmap_data,
        },
