...
export const callToken = async (tokenId, counterId = null) => {
  const params = counterId ? `?counter_id=${counterId}` : ''
  const response = await api.post(`/tokens/${tokenId}/call${params}`)
  return response.data
}
...
export const holdToken = async (tokenId) => {
  const response = await api.post(`/tokens/${tokenId}/hold`)
  return response.data
}

export const resumeToken = async (tokenId) => {
  const response = await api.post(`/tokens/${tokenId}/resume`)
  return response.data
}
